...
// ─── Date helpers (same as StatsSection) ─────────────────────────────────────
...
// ─── Monthly stats calculator (same logic as calcStats) ───────────────────────
const buildMonthlyData = (channels, txMap) => {
  const result = {};

  channels.forEach((ch) => {
    const createdD  = getCreatedDate(ch);
    const activityD = getActivityDate(ch, txMap);
    ...
    if (createdD) {
      const key = `${createdD.getFullYear()}-${createdD.getMonth() + 1}`;
      if (!result[key]) {
        result[key] = {
          month:       createdD.toLocaleString('default', { month: 'short', year: 'numeric' }),
          year:        createdD.getFullYear(),
          monthNumber: createdD.getMonth() + 1,
          sales: 0, purchases: 0, profit: 0,
        };
      }
      result[key].purchases += p;
    }

    // ── Sales + Profit: activityDate ke month mein ───────────────────────────
    const completedStatuses = new Set(['sold', 'terminatedWithLoss', 'hacked', 'terminatedWithoutLoss']);
    if (!completedStatuses.has(ch.status) || !activityD) return;

    const key = `${activityD.getFullYear()}-${activityD.getMonth() + 1}`;
    if (!result[key]) {
      result[key] = {
        month:       activityD.toLocaleString('default', { month: 'short', year: 'numeric' }),
        year:        activityD.getFullYear(),
        monthNumber: activityD.getMonth() + 1,
        sales: 0, purchases: 0, profit: 0,
      };
    }

    // Exactly same as calcStats
    switch (ch.status) {
      case 'sold':
        result[key].sales  += s;
        result[key].profit += s - p;
        break;
  ...
  // Sort: oldest → newest (chart left to right)
...
export default function ChartsSection({ darkMode }) {
  ...
  // Same txMap as StatsSection
  const txMap = useMemo(() => {
    const map = new Map();
    transactions.forEach((tx) => {
      if (tx.channelId) map.set(tx.channelId, tx);
    });
    return map;
  }, [transactions]);
  ...
  const monthlyData = useMemo(() => {
    ...
    return buildMonthlyData(channels, txMap);
  }, [channels, txMap]);
  ...
  const purchasesData = monthlyData.map((d) => d.purchases);
  ...
  const avgSale     = salesData.length
    ? Math.round(salesData.reduce((a, b) => a + b, 0) / salesData.length) : 0;
  ...
  const barOptions = {
    ...
    fill: {
      type: "gradient",
      gradient: { shade: "dark", type: "vertical", shadeIntensity: 0.3, opacityFrom: 1, opacityTo: 0.75, stops: [0, 100] },
    },
    plotOptions: {
      bar: { borderRadius: 8, columnWidth: "52%", distributed: true, dataLabels: { position: "top" } },
    },
    dataLabels: {
      enabled: true,
      formatter: (val) =>
    ...
    annotations: {
      yaxis: [{
        y: 0, borderColor: "#9ca3af", strokeDashArray: 4,
        label: { text: "Break Even", style: { color: textColor, fontSize: "10px", background: "transparent" } },
      }],
    },
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <motion.div
        ...
        whileHover={{ scale: 1.005, y: -2 }} transition={{ delay: 0.3, duration: 0.5 }}
        className="bg-white dark:bg-gray-900 rounded-2xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm transition-all"
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-200">Monthly Sales vs Purchases</h3>
        ...
        <ReactApexChart
          options={areaOptions}
          series={[{ name: "Sales", data: salesData }, { name: "Purchases", data: purchasesData }]}
      ...
      <motion.div
        initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }}
        whileHover={{ scale: 1.005, y: -2 }} transition={{ delay: 0.4, duration: 0.5 }}
        className="bg-white dark:bg-gray-900 rounded-2xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm transition-all"
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-200">Monthly Profit</h3>
          <span className="text-xs text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-800 px-2 py-1 rounded-lg">
            {months.length} Month{months.length !== 1 ? "s" : ""}
          </span>
        </div>
