...
function fmtRs(n) { return "Rs " + n.toLocaleString(); }
  ...
  return (
    ...
    >
      ...
      <div className={`h-20 bg-gradient-to-br ${avatarColor(ch.id)} flex items-center justify-center`}>
        <span className="text-2xl font-bold text-white tracking-wide">{getInitials(ch.channelName)}</span>
      </div>

      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-2">
          <h3 className="font-semibold text-gray-800 dark:text-white text-sm leading-tight line-clamp-1">{ch.channelName}</h3>
          <span className={`shrink-0 text-[10px] font-semibold px-2 py-0.5 rounded-full ${CAT_COLORS[ch.category]}`}>{ch.category}</span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 text-xs mb-3">
          ...
          <div>
            ...
            <p className="font-semibold text-gray-700 dark:text-gray-200">{fmtRs(ch.purchasePrice)}</p>
          </div>
          <div>
            <p className="text-gray-400 dark:text-gray-500">Sale Price</p>
            <p className="font-semibold text-emerald-600 dark:text-emerald-400">{fmtRs(ch.salePrice)}</p>
          </div>
        </div>
...
export default function Channels({ channels, setChannels }) {
  ...
  const clearAll = () => {
    setSearchName(""); setSearchMail(""); setCategory("All");
    setMonoFilter("All"); setDateFilter("all"); setDateMonth(""); setPage(1);
  };
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-400 dark:text-gray-500">
          ...
          </span> of <span className="font-semibold text-gray-600 dark:text-gray-300">{filtered.length}</span> channels
        </p>
      ...
      <AnimatePresence mode="wait">
        ...
        {filtered.length === 0 && (
          ...
          >
            ...
            <button onClick={clearAll} className="text-sm text-emerald-500 hover:underline font-medium">Clear filters</button>
          </motion.div>
        )}
                    ...
                    >
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          <div className={`w-8 h-8 rounded-xl bg-gradient-to-br ${avatarColor(ch.id)} flex items-center justify-center text-white text-xs font-bold shrink-0`}>
                      ...
                      <td className="px-4 py-3 text-gray-600 dark:text-gray-300 whitespace-nowrap">{fmtRs(ch.purchasePrice)}</td>
